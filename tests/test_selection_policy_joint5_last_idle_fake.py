"""Fake test for the last fixed candidate `joint5`."""
...
class SelectionPolicyJoint5LastIdleFakeTest(unittest.TestCase):
    """Verify the fixed traversal falls through to `joint5` when needed."""

    def test_joint5_selected_when_it_is_the_only_idle_joint(self) -> None:
        estimate_bundle = _build_estimate_bundle(idle_joint_ids={"joint5"})

        result = find_first_idle_joint_from_joint1_to_joint5(estimate_bundle)

        self.assertEqual(result.selected_joint_id, "joint5")
        self.assertEqual(result.selected_joint_index, 5)
        self.assertFalse(result.direct_front_cooperation)
        self.assertTrue(result.requires_recursive_transfer)
        self.assertEqual(result.diagnostics["idle_joint_ids"], ["joint5"])
        self.assertEqual(
