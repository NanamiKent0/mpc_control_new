"""Fake end-to-end test for recursive transfer followed by front cooperation."""
...
class TurnIntentDrivesRecursiveTransferFakeTest(unittest.TestCase):
    """Verify `TIP_TURN(-15)` uses recursive transfer before front cooperation."""

    def test_tip_turn_recursive_path_converges_then_returns_to_free_growth(self) -> None:
        compile_frame = build_turn_runtime_frame(idle_joint_ids={"joint4"})
        local_transfer_frame = compile_frame
        settled_frame = build_turn_runtime_frame(idle_joint_ids={"joint1", "joint4"})
        provider = SequenceProvider(
            [
                local_transfer_frame,
                local_transfer_frame,
                local_transfer_frame,
                local_transfer_frame,
                settled_frame,
        ...
        graph = session.submit_intent(tip_turn_intent(-15.0), frame=compile_frame)
        provider.frames = [
            local_transfer_frame,
            local_transfer_frame,
            local_transfer_frame,
        ...
        results = [session.step() for _ in range(7)]

        self.assertEqual(graph.metadata["selected_joint_id"], "joint4")
        self.assertEqual(graph.metadata["planner_mode"], "recursive")
        self.assertEqual(
            [result.diagnostics["current_plan_node_kind"] for result in results[:5]],
            [
                "local_transfer",
                "local_transfer",
                "local_transfer",
        ...
        self.assertAlmostEqual(
            abs(float(results[3].scheduler_step_result.skill_result.diagnostics["heading_error_deg"])),
            15.0,
            places=3,
        )
